 ...
 * $Id: ul_log.cpp,v 1.5.22.2 2010/04/15 06:38:17 zhang_rui Exp $ 
 * 
 **************************************************************************/
...
int ul_log_enabled(int level)
{
	...
	int self_log_id = level&UL_LOG_SELF_MASK;
	if (level >= UL_LOG_SELF_BEGIN && level <= UL_LOG_SELF_END && log_fd->spf[self_log_id] != NULL) {
		return 1;
	} else if (log_fd->mask < level) {
		return 0;
	} else {
		return 1;
	}
