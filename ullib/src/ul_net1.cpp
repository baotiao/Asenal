...
/* judge the ip is in the sunbet given by subnet 
 ...
 */
static int
is_in_subnet(const char *ip, const char *subnet)
{
	int i;
	const char *bip = ip;
...
is_ip_allowed(const char *ip, const char *pattern)
{
	...
	while ((bsnet != NULL) && (bsnet[0] != 0)) {
		...
		if (end != NULL) {
			...
			if (is_in_subnet(ip, subnet) == 1) {
				return 1;
			}
			bsnet = end + 1;
		} else {
			if (is_in_subnet(ip, bsnet) == 1) {
				return 1;
			}
			bsnet = NULL;
		}
...
static void ul_sub_timeval(struct timeval *tv, struct timeval *before,
	struct timeval *after)
{
	int64_t diff_us = (after->tv_sec - before->tv_sec) * 1000000
		+ (after->tv_usec - before->tv_usec);
	int64_t tv_us = tv->tv_sec * 1000000 + tv->tv_usec;
	if (tv_us > diff_us) {
		tv_us -= diff_us;
	} else {
		tv_us = 0;
 ...
 * 进行处理,以便再次读写
 *
 * @param [in] ve   : 需要处理的struct iovec *
 * @param [in/out] count   : struct iovec的个数
 * @param [in] offset   : 偏移
 * @return  处理的结果 
**/
static struct iovec *
